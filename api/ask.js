...
async function _askRaw(region, vkey, locale, question, history) {
  const v = vertical(region, vkey); if (!v) throw new Error('Unknown vertical');
  const lang = localeName(region, locale);
...
/* ---- first-turn answer cache (suggested questions / keyword queries are instant for everyone) ---- */
export async function ask(region, vkey, locale, question, history){
  const hasHist = Array.isArray(history) && history.length > 0;
  const nq = String(question||'').toLowerCase().replace(/\s+/g,' ').trim().slice(0,180);
  const ck = 'askc:'+locale+':'+region+':'+vkey+':'+nq;
  if(!hasHist){ try{ const c = await cacheGet(ck); if(c && c.answer) return c; }catch(e){} }
  const out = await _askRaw(region, vkey, locale, question, history);
  if(!hasHist && out && out.answer){ try{ await cacheSet(ck, out); }catch(e){} }
  return out;
}
